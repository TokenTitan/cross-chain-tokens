...
contract LZEndpointMock {
    ...
    function receivePayload(uint16 _srcChainId, bytes calldata _path, address _dstAddress, uint64 _nonce, uint _gasLimit, bytes calldata _payload) external receiveNonReentrant {
        ...
        if (sp.payloadHash != bytes32(0)) {
            ...
            try ILayerZeroReceiverUpgradeable(_dstAddress).lzReceive{gas: _gasLimit}(_srcChainId, _path, _nonce, _payload) {} catch (bytes memory reason) {
                storedPayload[_srcChainId][_path] = StoredPayload(uint64(_payload.length), _dstAddress, keccak256(_payload));
    ...
    function retryPayload(uint16 _srcChainId, bytes calldata _path, bytes calldata _payload) external {
        StoredPayload storage sp = storedPayload[_srcChainId][_path];
    ...
    function _clearMsgQue(uint16 _srcChainId, bytes calldata _path) internal {
        ...
        while (msgs.length > 0) {
            ...
            ILayerZeroReceiverUpgradeable(payload.dstAddress).lzReceive(_srcChainId, _path, payload.nonce, payload.payload);
            msgs.pop();
        }
    }
